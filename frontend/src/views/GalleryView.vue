...
watch(activeCategory, () => refresh(1).catch(() => {}))
...
<template>
  <section>
    ...
    <ImageGrid :images="images" :show-delete="false" />
    ...
    <div v-if="!loading && !images.length && !keyword && !error" class="empty-state">
      ...
      <h3>还没有图片</h3>
      <p>图床还是空的，上传第一张图片来开始吧</p>
      <RouterLink to="/manage" class="empty-action">前往管理页上传</RouterLink>
    </div>
